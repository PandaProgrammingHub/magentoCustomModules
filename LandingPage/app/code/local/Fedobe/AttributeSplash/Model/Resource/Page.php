...
//        echo $collection->getSelect()->__toString();exit;
    ...
    public function prepareFormattedConditions($collection, $storeId) {
        ...
        $stock_filter = $custom_qty_filter = $custom_final_price = $custom_state_filter = $product_new_from = 0;
        ...
        if (!empty($opr)) {
            foreach ($opr as $k => $v) {
                ...
                $product_new_from = $anddata['product_new_from'][0];
                $productnewfilter = $anddata['product_new_from'][1];
            }
            ...
            //Here for custom new product filter
            if ($product_new_from) {
                $this->addCustomNewProductFilter($collection, $productnewfilter['operator'], $productnewfilter['value']);
            }
        }
        return $collection;
    }
    ...
    public function commonAndOperation($cnd, $k, $collection) {
        $stock_filter = $custom_qty_filter = $custom_final_price = $custom_state_filter = $product_new_from = 0;
        $qtyfilter = $fpricefilter = $csstatefilter = $productnewfilter = array();
        foreach ($cnd[$k]['cond'] as $ick => $icv) {
            ...
            if ($icv['attribute'] == 'product_new_from') {
                $product_new_from = 1;
                $productnewfilter['operator'] = $icv['operator'];
                $productnewfilter['value'] = $icv['value'];
                continue;
            }
...
//echo $collection->getSelect();exit;
        ...
        $returarr = array(
            ...
            "product_new_from" => array($product_new_from, $productnewfilter)
        );
        return $returarr;
    }
    ...
    public function commonOrOperation($cnd, $k, $collection) {
        $stock_filter = $custom_qty_filter = $custom_final_price = $custom_state_filter = $product_new_from = 0;
        $condarr = $qtyfilter = $fpricefilter = $csstatefilter = $productnewfilter = array();
        foreach ($cnd[$k]['cond'] as $ick => $icv) {
            if ($icv['attribute'] == 'custom_stock') {
                $stock_filter = 1;
                continue;
    ...
    public function addCustomNewProductFilter($collection, $opr, $val) {
        $days = "-".$val." day";
        $todayDate = Mage::app()->getLocale()->date()->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
        $todayDate = date('Y-m-d',(strtotime ($days, strtotime ( $todayDate))));
        $collection->addAttributeToFilter(array(
                                                                 array('attribute'=> 'created_at','gteq' => $todayDate),
                                                                 array('attribute'=> 'updated_at','gteq' => $todayDate),
                                                               )
                                          );
